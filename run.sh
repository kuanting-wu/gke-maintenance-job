...
THREE_WEEKS_LATER=$(date -d "+21 days" +%Y-%m-%dT%H:%M:%S%z | sed 's/\([0-9][0-9]\)$/:\1/')
...
EXCLUSION_END="${THREE_WEEKS_LATER}"
...
if gcloud container clusters describe "$GKE_CLUSTER_NAME" \
  ...
  --project="$PROJECT_ID" \
  --format="json" | grep -q "$EXCLUSION_NAME"; then
  echo "Exclusion '$EXCLUSION_NAME' exists. Removing it before re-adding..."
  gcloud container clusters update "$GKE_CLUSTER_NAME" \
    --region="$GKE_CLUSTER_REGION" \
    --project="$PROJECT_ID" \
    --remove-maintenance-exclusion="$EXCLUSION_NAME"
else
  echo "No existing exclusion named '$EXCLUSION_NAME' found."
fi
...
# This is the core command that updates your GKE cluster.
gcloud container clusters update "${GKE_CLUSTER_NAME}" \
    --region="${GKE_CLUSTER_REGION}" \
    --project="${PROJECT_ID}" \
    ...
    --add-maintenance-exclusion-scope="${EXCLUSION_SCOPE}"
